<?php
    ...
    if(empty($usuario) or empty($password) or empty($correo)){
      echo '<script> alert("Por favor, completa todos los datos"); window.history.back();</script>';
        ...
        try {
            $conexion = new PDO('mysql:host:8080=localhost:8080;dbname=salud_alimenticia', 'root', '');
        } catch (PDOException $e) {
            echo "Error: " . $e->getMessage();
        }

        $statement = $conexion->prepare('SELECT * FROM sesion WHERE Usuario = :usuario LIMIT 1');
    ...
    if($errores == ''){
        $statement = $conexion->prepare('INSERT INTO sesion (Folio, Usuario, Correo, Pass) VALUES (null, :usuario, :correo, :pass)');
